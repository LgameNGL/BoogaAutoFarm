--[[
OPEN SOURCE NOW!!!
**READ ME SO YOU UNDERSTAND HOW THIS WORKS**
How to use because it's a little more manual than you think.
Some things to know:
This will automatically eat when you get below 50 hunger. Will Eat Apples first.

This also automatically farms appples so you don't starve to death while farming.

**RUN THESE BELOW, BEFORE EXECUTING THIS ONE**
This means you'll need to place structures. Use the script below (You'll have to be standing where this image: https://imgur.com/a/EPhHKQx depicts)
loadstring(game:HttpGet("https://raw.githubusercontent.com/LgameNGL/BoogaAutoFarm/main/AutoFarmStructures.lua"))

To get to sky island, execute this script as well. Will automatically teleport you to Ancient Island.
loadstring(game:HttpGet("https://raw.githubusercontent.com/LgameNGL/BoogaAutoFarm/main/tptoskyisland.lua"))

use an auto clicker set to middle mouse / right mouse button so you wont get kicked for idle.

AND FINALLY, this script is not perfect because frankly, I do not care. You might get stuck some places and you might even get kicked by anti-cheat.
This is not recommended to be reliable for over night farming. However, it has worked.

As of right now, it does not automatically rebirth you so yeah. Make sure the tool you are using is out.
...
WARNING.. UGLY CODE BELOW. Feel free to change whatever because I'm still learning.

First ever proper script I made for a game, I don't think it's half bad.
...
_G.tree = false
_G.ping = 1000 -- How high your ping has to be to stop.
local amt = 1
_G.Farming = true
_G.Pickup = true

-- thanks to char for this
...
local createnotif = function(text,color,duration,delayTime)
	...
	local fade = 2
	local howmanyprompts = #mainGui.Subordinates.Prompts.Feed:GetChildren()-2
	if howmanyprompts >= maxPrompts then
		local bottomNote,noteHiearchy = nil,math.huge
		for _,v in next,mainGui.Subordinates.Prompts.Feed:GetChildren() do
			if v:IsA("TextLabel") then
				if v.LayoutOrder<noteHiearchy then
					bottomNote,noteHiearchy = v,v.LayoutOrder
				end
			end
		end
		if bottomNote then
	...
	notification.LayoutOrder = noteSerials
	noteSerials = noteSerials+1


	notification.Parent = mainGui.Subordinates.Prompts.Feed
	notification.Visible = true
	debris:AddItem(notification,duration+fade)

	local tweenInfo = TweenInfo.new(fade,Enum.EasingStyle.Linear,Enum.EasingDirection.InOut,0,false,duration or 1)
	local goals = {
...
rep.Events.UpdateData.OnClientEvent:connect(function(newData)	
	_G.data  = newData
end);
...
local Pickup = function(Character)
	...
	for i,v in pairs(workspace:GetChildren()) do
		...
		elseif v:FindFirstChild("Pickup") ~= nil and v:FindFirstChild("Part") ~= nil and v.Part.ClassName == "Part" and v.Name == "Essence" then
...
local getClosestTree = function(number)
	for i,v in pairs(game.Workspace.Resources:GetChildren()) do
		...
		if v.Name == "Sun Tree" or v.Name == "Sun Shrub" and v.Parent ~= "Contents" then
			if v.Name == "Sun Shrub" then partpos = "Stump" elseif v.Name == "Sun Tree" then partpos = "Reference" end
...
local StopCrash = coroutine.create(function()
...
if antilag2 then
	...
	if _G.Pickup then
						for _, v in pairs(workspace:GetChildren()) do
							if v.Name == "Apple Tree" and (Player1.Character.Head.Position - v.PrimaryPart.Position).magnitude < 40 then
						...
						for _, v in pairs(workspace.Deployables:GetChildren()) do
							if v.Name == "Plant Box" and (Player1.Character.Head.Position - v.PrimaryPart.Position).magnitude < 40 then
								if CanBearLoad("Apple Tree") then
...
local AutoPickup = coroutine.create(function()
	while true do
		wait(0.001)
...
local hitancientfast = coroutine.create(function()
	while true do
		wait()
		if AncientTree() then
			game.ReplicatedStorage.Events.SwingTool:FireServer({game:GetService("Workspace").Resources["Ancient Tree"].Reference, game:GetService("Workspace").Resources["Ancient Tree"].Trunk})
		end
	end
end)
...
createnotif("Auto farm Loaded... dead game though", Color3.fromRGB(0, 255, 0), 5,0)
